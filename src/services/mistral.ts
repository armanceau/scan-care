...
function cleanPrescriptionData(data: any): PrescriptionAnalysis {
  ...
  const medications = Array.isArray(data.medications)
    ? data.medications.map((med: any) => ({
        ...
        frequency:
          String(med.frequency || "").trim() || "Fréquence non spécifiée",
...
Si aucun médicament n'est détecté, retourne : {"medications": []}`;
    ...
    try {
      ...
      console.log(
        `✅ Analyse terminée : ${result.medications.length} médicament(s) détecté(s)`,
      );
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 401) {
        ...
        throw new Error(
          "Requête invalide: " + JSON.stringify(error.response.data),
        );
      } else {
        throw new Error(
          "Erreur API Mistral: " +
